...
public class MergingCarBehavior : MonoBehaviour
{
    ...
    [Tooltip("Assign the TurnSignalBlinkerSimpleV2 here, or leave blank to auto-find in children.")]
    public TurnSignalBlinkerSimpleV2 turnSignalBlinker;
    ...
    void Awake()
    {
        ...
        if (rb == null)
            Debug.LogError($"{nameof(MergingCarBehavior)} requires a Rigidbody on the same GameObject.");
        ...
        // Auto-find blinker if not assigned
        if (turnSignalBlinker == null)
            turnSignalBlinker = GetComponentInChildren<TurnSignalBlinkerSimpleV2>(true);
    }
    ...
    void FixedUpdate()
    {
        ...
        switch (phase)
        {
            ...
            case BotPhase.Merge_Lateral:
            {
                ...
                float t = 1f;
                if (totalDx > 0.0001f)
                    t = Mathf.Clamp01(1f - (remainingDx / totalDx));
                ...
                if (doneX)
                {
                    ...
                    if (turnSignalBlinker != null)
                        turnSignalBlinker.OnMergeEnded();
                }
